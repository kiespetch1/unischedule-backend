using UniSchedule.Abstractions.Messaging;
using UniSchedule.Identity.DTO.Messages.Groups;

namespace UniSchedule.Messaging.Consumers.Groups;

/// <summary>
///     Описание для обработчика сообщений
/// </summary>
public class SyncGroupsConsumerDefinition : ConsumerDefinitionBase<SyncGroupsConsumer, GroupsMqSyncParameters>
{
    /// <summary />
    public SyncGroupsConsumerDefinition() : base("sync_groups") { }
}